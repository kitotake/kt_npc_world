-- FIX v1.2 :
--   • zoneMod appliqué indépendamment de la proximité du joueur (avec coefficient réduit à distance)
--   • NPCs morts ignorés proprement (état "dead" posé, cleanup différé)
--   • Contagion de peur utilise une liste pre-collectée pour éviter O(N²) sur ActiveNPCs modifié
...
AddEventHandler("npc:update_emotion", function()
    ...
    -- FIX: pré-collecter les NPCs paniqués pour la contagion (évite boucle imbriquée live)
    local panicked = {}
    ...
    for id, npc in pairs(ActiveNPCs) do
        ...
        -- FIX: ignorer les NPCs morts (IsEntityDead) — ils seront nettoyés au prochain cleanup
        elseif IsEntityDead(npc.ped) then
            ...
            if dist < cfg.proximityRange then
                ...
                -- FIX: décroissance + zoneMod atténué même à distance
                -- Un gang dans sa zone reste agressif indépendamment du joueur
    ...
    -- FIX: contagion sur liste pré-collectée, pas de boucle imbriquée sur ActiveNPCs live
    if #panicked > 0 then
    ...
    -- Nettoyage différé des entités disparues détectées pendant la boucle
